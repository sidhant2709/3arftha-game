  ...
  try {
    ...
    document
      ...
      .forEach((teamElement) => {
        ...
        const participantCountInput = teamElement.querySelector(".newgame3_increase_decrease input");
    ...
    const requestBody = {
      subcategories: ["67c2f5aa1145a1aba2e7ea83", "67c2f5c81145a1aba2e7ea87"],
      teams: teams,
    };
    const response = await fetch(`${BASE_URL}/trial-game/${userId}`, {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      sessionStorage.setItem("gameId", result.trialGameId);
      sessionStorage.setItem("resultGameId", result.trialGameId);
      ...
      window.location.href = "trial_game_question.html";
    } else {
