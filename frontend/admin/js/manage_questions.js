document.addEventListener("DOMContentLoaded", function () {
  ...
  const totalQuestionButton = document.querySelector(
    ".total-question-btn small"
  );
    ...
    questions.forEach((question) => {
      ...
      row.innerHTML = `
        ...
        <td>
          ...
          <span class="table-icon delete-icon" data-id="${question._id}">
            <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  tableBody.addEventListener("click", async (event) => {
    const target = event.target.closest(".table-icon");
    if (target) {
      ...
      /*
        if (target.classList.contains("edit-icon")) {
          ...
          // Make an API call to fetch question details for editing
          try {
            const response = await fetch(`${apiUrl}/${questionId}`, {
              method: "PUT",
              headers: {
            ...
            console.log("Question data for editing:", questionData);
          } catch (error) {
